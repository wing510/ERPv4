/**
 * Phase 4 slice2 - AR payment POST/VOID RPC atomic tx (DEV only)
 *
 * Run: node tests/p4-ar-payment-rpc.test.mjs
 * 前置 SQL：v4.3.10 → v4.3.11
 */
...
const PREFIX = "TESTP4B-" + RUN_TS.slice(0, 19).replace(/T/g, "");
const ACTOR = "test-runner";
const SESSION = { role: "CEO", allowed_modules: "*" };
const AR_AMOUNT = 1000;
const PAY_AMOUNT = 400;

const evidenceRoot = path.resolve("tests/_p4b_evidence", PREFIX);
...
async function rpcReady_() {
  const { error } = await sb.rpc("erp_ar_post_payment_phase4_tx", {
    p_payment_id: "__probe__",
    p_ar_id: "__probe__",
...
async function ensureCustomerAndAr() {
  ...
  await masterCrudHandlers().create_customer({
    ...
    customer_name: "TEST P4B Customer",
...
async function main() {
  if (!(await rpcReady_())) {
    console.error("SKIP: erp_ar_post_payment_phase4_tx not deployed. Run v4.3.11 first.");
    process.exit(2);
  }
      ...
      JSON.stringify(
        {
          post_pass: postPass,
          void_pass: voidPass,
          payment_rpc: regRes.payment_rpc === true,
          void_rpc: voidRes.void_rpc === true
        },
        null,
        2
      )
