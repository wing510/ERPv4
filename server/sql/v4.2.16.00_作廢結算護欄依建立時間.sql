-- v4.2.16.00：作廢寄賣結算護欄 — 僅鎖「月結統計／回饋產生前」已存在的結算
-- 月結過帳後新增的結算（未計入該次月結）仍可作廢
...
declare
  ...
  v_period text;
  v_stl_created timestamptz;
  v_rebate_id text;
  v_rebate_created timestamptz;
  v_stat_id text;
  v_stat_created timestamptz;
begin
  ...
  select rebate_id, created_at
  into v_rebate_id, v_rebate_created
  ...
  if v_rebate_id is not null then
    if v_stl_created is null or v_rebate_created is null or v_stl_created < v_rebate_created then
      raise exception '此客戶 % 已產生月結回饋（%）。請先到「月結統計／回饋」作廢該筆回饋後，再作廢寄賣結算。', v_period, v_rebate_id;
    end if;
  end if;

  select stat_id, created_at
  into v_stat_id, v_stat_created
  ...
  if v_stat_id is not null then
    if v_stl_created is null or v_stat_created is null or v_stl_created < v_stat_created then
      raise exception '此客戶 % 已產生月結統計（%）。請先到「月結統計」作廢該筆後，再作廢寄賣結算。', v_period, v_stat_id;
    end if;
  end if;
end;
$$;

comment on function public.erp_assert_no_dealer_rebate_lock_for_settlement(text, text, date, text)
  is 'v4.2.16 作廢結算前檢查：僅鎖月結產生前已存在的結算';

-- 更新 RPC：傳入 settlement_id 供護欄比對建立時間
...
revoke all on function public.erp_assert_no_dealer_rebate_lock_for_settlement(text, text, date, text) from public;
grant execute on function public.erp_assert_no_dealer_rebate_lock_for_settlement(text, text, date, text) to service_role;
